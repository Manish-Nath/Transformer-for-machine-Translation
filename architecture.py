...
class Positional_encoding(nn.Module):
    ...
    def forward(self, x):
        x = x + self.pe[:, :x.shape[1], :].requires_grad_(False)  # up to x.shape[1] - seq_len add this
        return self.dropout(x)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        # x shape: [batch, head, seq_len, d_k] -> [batch, seq_len, head, d_k]
...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        return self.dropout(sublayer(self.norm(x))) + x  # Adding the input x to the output for residual connection
